						...
						if(args[1].equalsIgnoreCase ("player")) {
							if(plugin.permission.has(sender, "bounties.bountycreate.player")) { //Permissions Check
								...
								return true;
									...
									if(accntBalance<500+bountyamount) {
										...
										completetime = System.currentTimeMillis() + 43200000; 
										if (plugin.MySQL = true) {
						 					String query = "INSERT INTO bountiesplayer (target,poster,amount,killcount,posttime,endtime) VALUES ('" 
						 							+ wantedPlayerName + "', '" + posterPlayerName + "', '" + bountyamount + "', '" 
						 							+ killcount + "', " + time + ", " + completetime + ")";
						 					try {
							...
							if(plugin.permission.has(sender, "bounties.bountycreate.faction")) {
								...
								if(Factions.i.isTagTaken(factionName)) {
									...
									completetime = System.currentTimeMillis() + 43200000; 
									if (plugin.MySQL = true) {
					 					String query = "INSERT INTO bountiesfaction (target,poster,amount,killcount,posttime,endtime) VALUES ('" 
					 							+ factionName + "', '" + posterPlayerName + "', '" + bountyamount + "', '" 
					 							+ killcount + "', " + time + ", " + completetime + ")";
					 					try {
					 						sqlHandler.executeQuery(query, true);
					 					} catch (SQLException e) {
